...
public:
    Rectangle(Vector position, float width, float height, Material material) 
    ...
    void printInfo(std::ostream& out) const {
        out << "Rectangle at: (" << this->getPosition().transpose() << ") with dimensions: " << width_ << "x" << height_ << ", with material: " << this->getMaterial().name << std::endl;
    }
